...
import { correctionSessionService } from "@/services/session/session.service"
...
export const useStartCorrection = () => {
  ...
  const onSubmit: SubmitHandler<IFormInput> = (data) => {
    ...
    startIsLoadingEdit(async () => {
      ...
      const title = "Matnni to'g'irlash sessiyasi"
      const newSessionId = await correctionSessionService.startSession(
        text,
        title
      )
      router.push(`${PROTECTED_PAGES.CORRECTION}/${newSessionId}`)
    })
  }
  ...
  useEffect(() => {
    setCurrentAi(ECurrentAI.CORRECTION)
  }, [setCurrentAi])
